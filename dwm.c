...
static void spawnbelow(const Arg *arg);
...
moveresizeedge(const Arg *arg)
{
	...
	/* maximum auto-resize */
	enum { MaxFloatH = 720, MaxFloatW = 1280 };

	/* vertical */
	if(e == 'v') {
		nw = c->w > MaxFloatW ? MaxFloatW : c->w;
		nh = selmon->mh - starty;
		nx = (selmon->mw - nw - 2 * c->bw) / 2;
		ny = starty;
	}
	...
	if(e == 'c') {
		nw = c->w > MaxFloatW ? MaxFloatW : c->w;
		nh = c->h > MaxFloatH ? MaxFloatH : c->h;
		nx = (selmon->mw - nw - 2 * c->bw) / 2;
		ny = (selmon->mh - nh - 2 * c->bw) / 2;
	}
	...
	if(e == 'p') {
		nw = c->w > MaxFloatW ? MaxFloatW : c->w;
		nh = c->h > MaxFloatH ? MaxFloatH : c->h;
		nx = selmon->mw - nw - 2 * c->bw;
		ny = selmon->mh - nh - 2 * c->bw;
	}
